...
exports.run = async (client, message, args) => {
        ...
        let izinli = ["913086095347023912","569167325736075264"] // kendi idni koy
        if (!izinli.includes(message.member.id)) return message.channel.send('Eval Komutunu Kullanabilmek için Sahibim Olmalısın')

        if (args[0] == '@everyone') return;

        if (!args[0]) {
            const embed = new Discord.MessageEmbed()
                .setDescription(`.eval client.emit("guildMemberAdd" , message.member)`)
                .setColor("RANDOM")
                .setTimestamp()
