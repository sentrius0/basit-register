...
return;
    ...
    db.push(`isimler_${member.id}`, ` \`${name} | ${age}\` (KIZ)`);
    ...
    await message.guild.members.cache.get(member.id).roles.add(ayarlar.kız)
    message.channel.send(embed.setDescription(`${member} üyesine  rolleri verilerek \`${name} | ${age}\` İsminde kayıt edildi! (KIZ)`)).then(x => x.delete({ timeout: 10000 }));
}
